...
namespace xUnit2.Demo.Tests.NSubstitute.Demo.LessonBase
{
    ...
    public class Lesson14
    {
        ...
        #region 当参数没有默认构造函数时引发事件

        //NSubstitute 不总是能够创建事件参数。
        //如果事件的参数没有默认的构造函数，你可能不得不使用 Raise.EventWith<TEventArgs>(...) 来创建事件参数，例如下面例子中的 LowFuelWarning 事件。
        //如果没有提供事件的发送者，则 NSubstitute 会创建。
        ...
        //这种事件可以通过使用 Raise.Event<TypeOfEventHandlerDelegate>(arguments)来引发。NSubstitute 会尝试和猜测该委托所需的参数，但如果没成功，NSubstitute 会告诉你具体需要提供哪个参数。
